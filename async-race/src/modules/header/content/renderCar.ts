...
export const renderCar = ({id, name, isEngineStarted, color}) => {
  ...
  startEngineBtn.id = `start-engine-car-${id}`;
  ...
  stopEngineBtn.id = `stop-engine-car-${id}`;
  ...
  car.append(carImage);
  launchPad.append(controlPanel, car, flag);
  road.append(launchPad);
